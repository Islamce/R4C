...
| Controlled document | Implemented with synthetic HTML preview only | `GET /quotations/:id/preview-document` returns `SYNTHETIC_HTML_DOCUMENT_PREVIEW` and a snapshot payload. | No generated, stored, or downloadable PDF artifact; the UI is explicitly labelled as a controlled HTML document preview. |
