...
export function BuyerQuotationExperience({ token, preview, previewState }: { token: string; preview: boolean; previewState?: "expired" }) {
  ...
  const decisionHint = decision === "ACCEPTED" ? copy.acceptedHint : decision === "DECLINED" ? copy.declinedHint : decision === "CLARIFICATION_REQUESTED" ? copy.clarificationHint : copy.decisionPrompt;
