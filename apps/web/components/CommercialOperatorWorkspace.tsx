          ...
          <>
            <section className="commercial-journey-grid">
              ...
              <section className="create-panel commercial-pipeline">
                ...
                {groupedLeads.length === 0 ? <div className="empty-state" role="status"><p>{t("commercial.noLeads")}</p><small>{t("commercial.noLeadsHint")}</small></div> : groupedLeads.map(([status, rows]) => <div className="lead-stage" key={status}><h3><span>{t(`commercial.status.${status}` as "commercial.status.NEW")}</span><small>{rows.length}</small></h3>{rows.map((lead) => <button type="button" className={selectedLead?.id === lead.id ? "lead-card lead-card-selected" : "lead-card"} key={lead.id} onClick={() => setSelectedLead(lead)}><strong>{lead.customer ? `${lead.customer.firstName} ${lead.customer.lastName ?? ""}` : lead.source}</strong><span>{lead.project?.name ?? lead.source}</span><small>{lead.assignedTo.displayName}</small></button>)}</div>)}
              </section>
            </section>
